...
pb_datacenter_inventory is a sample script to get an inventory overview of your data centers via Profitbricks REST-API.

It collects some basic information on
- reserved ip blocks:
    Location,RscType,RscID,State,Size,IP addresses
- images and snapshots
    Visibility(public|private),Location,RscType,SubType(CDROM|HDD),RscID,RscName,State,LicType,Size,Created,Modified
- servers
    DCID,DCName,Location,RscType,RscID,RscName,State,LicType,Cores,RAM,# NICs,# Volumes,(Total) Storage,---,Created,Modified
  and storage
    DCID,DCName,Location,RscType,RscID,RscName,State,LicType,---,---,---,---,(Total) Storage,Connected to,Created,Modified
- networking infrastructure (LANs/NICs)
    DCID,DCName,Location,LAN ID,LAN name,public?,State,# NICs,NIC ID,MAC address,DHCP?,IP(s),NIC name,Firewall?,Connected to,ID,Name
from all your datacenters.
This data is written to a resource specific CSV file which can be used for reports or automatic post processing.
...
def get_dc_inventory(pbclient, dc=None) :
    ...
    for server in servers['items'] :
        ...
        # OS is determined by boot device (volume||cdrom), not a server property. Might even be unspecified
        bootOS = "NONE"
        ...
        for vol in server_vols :
            ...
            if verbose:
                print("volume %s is connected to %s w/ OS %s" % (vol['id'],bound_vols[vol['id']], licence_type))
        server_nics = server['entities']['nics']['items']
        n_nics = len(server_nics)
        server_data += [n_nics, n_volumes, total_disk, "",
    ...
    for volume in volumes['items'] :
        ...
        volid = volume['id']
        vol_data = [volume['type'], volid, volume['properties']['name'], volume['metadata']['state'], 
...
def get_images(pbclient):
    ...
    for image in images['items']:
        ...
        img_data = [('public'  if image['properties']['public'] else 'private'), image['properties']['location'],
                    image['type'],  image['properties']['imageType'], image['id'], image['properties']['name'],
...
def get_snapshots(pbclient):
    ...
    for snap in snapshots['items']:
        ...
        snap_data = ["private", snap['properties']['location'],
...
def get_dc_network(pbclient, dc=None):
    ...
    for lan in lans['items']:
        if verbose > 1:
            print("LAN: %s" % str(lan))
        lan_data = dc_data + ["LAN "+lan['id'], lan['properties']['name'], lan['properties']['public'],
                              lan['metadata']['state']
                             ]
        ...
        if len(nics) > 0:
            for nic in nics:
                ...
                nic_data = [nic['id'], nic_props['mac'],nic_props['dhcp'], ips, nic_props['name'],
...
def get_requests(pbclient):
    ...
    for req in requests['items']:
        reqid = req['id']
        req_data = pbclient.get_request(reqid)   # False: get request (w/ body), True: get request status (DONE,..)
        req_stat = pbclient.get_request(reqid, True)   # False: get request (w/ body), True: get request status (DONE,..)
        if verbose > 2:
            print("REQ: %s\n     %s" % (str(req_data), str(req_stat)))
        if verbose > 1:
            print("request %s|%s|%s" % (str(reqid), str(req_data['metadata']), str(req_stat['metadata'])))
    # end for(requests)
# end get_requests()
